...
class LocalExactDaumHuangFlow(FlowFilterBase):
    ...
    def _compute_local_flow_params(
        ...
        try:
            ...
            S_inv_H = np.linalg.solve(L_S.T, np.linalg.solve(L_S, H_i))
        except np.linalg.LinAlgError:
            # Use pseudoinverse as robust fallback for ill-conditioned matrices
    ...
    def _flow_step_euler(
        ...
        # Update each particle with its own local linearization
        if self.n_threads and self.n_threads > 1:
            def update_particle(i):
                return self._update_single_particle(i, particles, y, lambda_val, d_lambda)

            with ThreadPoolExecutor(max_workers=self.n_threads) as executor:
                particles_new = np.array(list(executor.map(
                    update_particle, range(self.n_particles)
                )))
        else:
            for i in range(self.n_particles):
                particles_new[i] = self._update_single_particle(
                    i, particles, y, lambda_val, d_lambda
                )
    ...
    def update(self, y: np.ndarray):
        ...
        # FEEDBACK MECHANISM: Update global filter mean from ensemble
        # Again, do NOT update covariance - let global filter maintain its own
        if self.use_feedback:
            ensemble_mean = np.mean(self.particles, axis=0)
            self.global_filter.mean = ensemble_mean.copy()
