    ...
    case {'LEDH','LEDH_cluster'}
        switch ps.setup.example_name
            case 'Septier16'
                ...
                R_likeparams = ps.likeparams.R;
        end
        for particle_ix = 1:size(vg.xp,2)
            switch ps.setup.example_name
                case 'Septier16'
                    ps.likeparams.R = squeeze(R_likeparams(:,:,particle_ix));
            end
            switch ps.setup.kflag
                case 'EKF1'
%                     [vg.M_prior_all(:,particle_ix),] = ukf_predict1(vg.xp(:,particle_ix),squeeze(vg.PU_all(:,:,particle_ix)),@propparams_no_noise.propagatefcn,ps.propparams.Q,propparams_no_noise);                    
                    [~,vg.PU_all(:,:,particle_ix)] = ekf_update1(vg.M_prior_all(:,particle_ix),vg.PP_all(:,:,particle_ix),z_current,ps.likeparams.dh_dx_func,ps.likeparams.R,ps.likeparams.h_func,[],ps.likeparams);
                case 'UKF1'
                    [~,vg.PU_all(:,:,particle_ix)] = ukf_update1(vg.M_prior_all(:,particle_ix),vg.PP_all(:,:,particle_ix),z_current,ps.likeparams.dh_dx_func,ps.likeparams.R,ps.likeparams.h_func,[],ps.likeparams);
                otherwise
                    warning('unspecified covariance update type');
            end
                     
            [~,regind] = chol(squeeze(vg.PU_all(:,:,particle_ix)));
            if regind
                vg.PU_all(:,:,particle_ix) = cov_regularize(squeeze(vg.PU_all(:,:,particle_ix)));
            end
        end
end
