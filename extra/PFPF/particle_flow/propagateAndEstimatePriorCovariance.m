...
switch ps.setup.pf_type
    ...
    case {'LEDH','LEDH_cluster'}
        for particle_ix = 1:size(vg.xp,2)
            switch ps.setup.kflag
                case 'UKF1'
                    [vg.M_prior_all(:,particle_ix),vg.PP_all(:,:,particle_ix)] = ukf_predict1(vg.xp(:,particle_ix),squeeze(vg.PU_all(:,:,particle_ix)),@propparams_no_noise.propagatefcn,ps.propparams.Q,propparams_no_noise);
                otherwise% single-order EKF
                    [vg.M_prior_all(:,particle_ix),vg.PP_all(:,:,particle_ix)] = ekf_predict1(vg.xp(:,particle_ix),squeeze(vg.PU_all(:,:,particle_ix)),[],ps.propparams.Q,@propparams_no_noise.propagatefcn,[],propparams_no_noise);
            end
...
switch ps.setup.pf_type
    ...
    case {'LEDH','LEDH_cluster'}
        vg.mu_0_all = ps.propparams.propagatefcn(vg.xp,propparams_no_noise);
