...
class EDHParticleFlowFilter:
    ...
    def _compute_A_b(self, eta_bar: np.ndarray, eta_bar_0: np.ndarray,
        ...
        S_inv_H = safe_solve(S, H, method='default')
    ...
    def _compute_drift_batch(self, particles: np.ndarray, A: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    def update(self, y: np.ndarray):
        ...
        for j in range(self.n_lambda_steps):
            ...
            eta_bar = euler_step(eta_bar, self._compute_drift_single, epsilon_j, A, b)

            # Lines 15-17: Migrate particles (vectorized) using euler_step
            self.particles = euler_step(self.particles, self._compute_drift_batch, epsilon_j, A, b)
        ...
        # Use shared normalize_log_weights with clipping
        self.weights = normalize_log_weights(log_weights, clip_range=(-30, 30))
