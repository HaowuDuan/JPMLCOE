...
class LinearGaussianModel(StateSpaceModel):
    ...
    def state_transition_mean_batch(self, particles: np.ndarray) -> np.ndarray:
        """Vectorized state transition mean: F @ particles.T → transpose → (N, nx)."""
    ...
    def log_observation_prob_batch(self, observation: np.ndarray, particles: np.ndarray) -> np.ndarray:
        ...
        # Mean: H @ particles.T → (ny, N) → transpose → (N, ny)
        means = (self.H @ particles.T).T  # (N, ny)
        ...
        return -0.5 * (self.obs_dim * np.log(2 * np.pi) + logdet + mahalanobis)
