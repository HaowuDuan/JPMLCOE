...
def nonlinear_test_model():
    """Create a simple nonlinear model for testing EKF with mild nonlinearity."""
...
# ============================================================================
# TEST 8: NONLINEAR MODEL TESTING
# ============================================================================

class TestNonlinearModel:
    """Test EKF with mildly nonlinear transition and observation functions."""
    ...
    def test_nonlinear_predict(self, nonlinear_test_model):
        ...
        ekf = ExtendedKalmanFilter(model, mean_0=mean_0, Sigma_0=np.eye(2))
        
        ekf.predict()
        
        # Check that prediction includes the nonlinear term
        # f(x) = [x[0] + x[1]*dt + 0.1*sin(x[0]), x[1]]
        expected_mean_0 = mean_0[0] + mean_0[1] * model.dt + 0.1 * np.sin(mean_0[0])
        assert np.allclose(ekf.mean[0], expected_mean_0, rtol=1e-4)
    ...
    def test_nonlinear_update(self, nonlinear_test_model):
        ...
        # Update should reduce uncertainty
        assert np.trace(ekf.cov) < np.trace(cov_pred)
    ...
    def test_nonlinear_filter_sequence(self, nonlinear_test_model):
        ...
        model = nonlinear_test_model
        ekf = ExtendedKalmanFilter(model, mean_0=np.array([0.5, 0.2]))
    ...
    def test_nonlinear_jacobian_used(self, nonlinear_test_model):
        """Test that Jacobians with nonlinear terms are used correctly."""
        model = nonlinear_test_model
        mean_0 = np.array([1.0, 0.5])
        ekf = ExtendedKalmanFilter(model, mean_0=mean_0, Sigma_0=np.eye(2))
        
        # The Jacobian at mean_0 should include cos term
        F = model.jacobian_f(mean_0)
        expected_F_00 = 1.0 + 0.1 * np.cos(mean_0[0])
        assert np.allclose(F[0, 0], expected_F_00)
