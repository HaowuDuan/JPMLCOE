...
Financial systems are driven by latent factors that shape observable market data. Problems such as stochastic volatility {\color{red}1, what does it even mean by stochastic volatility?}, limit order book dynamics, or macroeconomic regime shifts are frequently cast as Hidden Markov Models (HMMs), which has discrete states, or continuous state-space models (SSMs). In this document, we focus on continuous state-space models (SSMs). \\[10pt]

Suppose we have some financial data {\color{red}2, cite some source where financial data is represented as a time series.} $\{z_k\}$, where $k$ is the time step. This is all we have. The real time evolution of economic factor which might be hidden and not observable is denoted as $\{x_k\}$, sometimes referred to ground truth in controlled experiments. As a one-dimensional demonstration, let us first write down a most generic state-space model,
\begin{equation}
...
\end{split}
\end{equation}
%where $k$ denotes the time step, $z_k$ is the observation, and $x_k$ is the unobserved latent state, sometimes referred to ground truth in controlled experiments.

$w_k$ and $u_k$ are mutually and temporally uncorrelated noises. The function $f$ governs state evolution and $h$ maps the states to observations. In practice we observe only $z_k$, which may have the same or lower dimension than $x_k$ (the latter case is called partial observation and makes inference substantially harder due to information loss). If we have reliable model that actually describe the laws of economics, then we can accurately make predictions about economic phenomena. A model consists two components, functional form and parameters. Both of which are unknown. In practice, what people do is to make anzatz about the functional form of the model, and fit the parameters of the model, until a better anzatz appears, the said anzatz will represent our best understanding of the phenomena. The empirical observation itself is agonistic to models. To make sure the methods we used can make solid inference of the model parameters, we study the model and tests the relavent numerical methods in controlled experiments. This is the focus of this project. We want to generate artificial data from known model and develop robust numerical methods to extract the parameters of the model. Since the models and parameters are known, there is a clear way to gauge successfulness of the methods. The methods we use to make estimation about the hidden states based on observation is known as \textbf{filtering}, and the methods for extracting the model parameter is \textbf{Baysian inference.}
...
%Successful application of state-space models rests on two pillars:
%\begin{enumerate}
  ...
  %  \item An effective inference algorithm that estimate the hidden state $x_k$ from the observation $z_k$ — here, filtering algorithm.
   % \end{enumerate}
%In recent years neural networks have entered both parts of the equation. We can replace the conventional maps with learned versions: $f \to f_\theta$ and $h \to h_\theta$. Neural filtering algorithms have also emerged. In this note, we started with more traditional methods, and focuses on building a robust pipeline. 

%This might be an incomplete or even biased take that neural network is icing on the cake instead of the backbone of Bayesian inference.
...
\subsubsection{Beysian inference}
...
\begin{equation}
    L_{MLE}(\theta) = -\sum_{k=1}^{T} \log \, p_\theta(z_k \mid z_{1:k-1})
\end{equation}
Since the logarithm is monotonically increasing, minimizing $L(\theta)$ maximizes the predictive density of each observation given past data~\cite{Särkkä2013BayesianFilteringAndSmoothing}. We denote the optimization to achieve minimum log likelihood as MLE (Maximum log-likelihood estimation). In a Bayesian setting, where parameters are given a prior $p(\theta)$, which add some hard requirement for the range of parameters,we instead minimize,
\begin{equation}
    L_{MAP}(\theta) = -\sum_{k=1}^{T} \log \, p_\theta(z_k \mid z_{1:k-1}) - \log p(\theta),
\end{equation}
which corresponds to maximum a posteriori (MAP) estimation. MAP estimation is more informed than MLE, because sometimes, we know for sure that certain parameter range is unreasonable, which could invalidate the model or causing numerical instability for example.
...
% \end{figure}
we see that range bearing model works with UKF and EKF because we can get reasonable Jacobian from the observation model.
...
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.9\textwidth]{stepsize_resampling.png}
%   \caption{HMC step size evolution on the cubic sensor model for three resampling methods. Systematic and soft resampling exhibit step size collapse due to the \texttt{tf.cond} threshold discontinuity, while OT Sinkhorn maintains a stable step size.}
%   \label{fig:stepsize_resampling}
% \end{figure}
...
This appendix details the four diagnostic quantities reported after every HMC/NUTS run.
