...
The setup is as following: we observe a time series $\{z_k\}$, $k = 1, 2, \ldots, T$. The real driver is a hidden state $\{x_k\}$ that we cannot observe directly. The most general SSM writes:
\begin{equation}
...
Having a good model is only half the battle; we also need a reliable implementation. I use synthetic models first because the truth is known. Given the state evolution $f$ and observation model $h$, the goal is to estimate the hidden state $x_k$ via the filtering distribution $p(x_k \mid z_{1:k})$. This is obtained recursively:
...
In financial applications (and most real-world settings), we do not aim to recover an exact deterministic value of $x_k$. Instead, the useful quantities are typically the mean $m_k$ and covariance $P_k$ of the filtering distribution. The filtering problem is generally a high-dimensional path integral and is intractable except in special cases (e.g., linear Gaussian SSMs). Different filtering methods (EKF, UKF, particle filters, etc.) use distinct approximations and mathematical frameworks. The filter has one job: approximate the posterior and stay numerically stable.
...
The 1D stationary vs.\ diffuse distinction above generalises to every model in this report and is worth stating once so later sections can refer back to it. For a multi-dimensional linear state $\mathbf{x}_k = \mathbf{F}\mathbf{x}_{k-1} + \boldsymbol{\omega}_{k-1}$ with stable $\mathbf{F}$ (spectral radius $<1$), the stationary covariance is the unique solution of the discrete Lyapunov equation $\boldsymbol{\Sigma}_0 = \mathbf{F}\boldsymbol{\Sigma}_0\mathbf{F}^\top + \mathbf{Q}$, and the natural prior is $\mathbf{x}_0 \sim \mathcal{N}(\mathbf{0}, \boldsymbol{\Sigma}_0)$. We use this for the linear Gaussian benchmarks and for the 1D stochastic volatility model, where $|\alpha| < 1$ gives $\Sigma_0 = \sigma^2/(1-\alpha^2)$ directly. For tracking models whose dynamics are not contractive --- the range-bearing and acoustic-tracking problems --- there is no Lyapunov solution and we fall back on a physically motivated prior: a mean $\boldsymbol{\mu}_0$ inside the typical observation footprint and an isotropic covariance $\boldsymbol{\Sigma}_0 = I$ large enough to cover plausible starting positions. On range-bearing, the canonical choice in this report is $\boldsymbol{\mu}_0 = (5, 5)^\top$ with $\boldsymbol{\Sigma}_0 = I_2$, as stated in \S\ref{par:range-bearing-model}; this keeps the initial bearing Jacobian away from the sensor singularity. Individual model sections below give specific numerical values but refer back to this paragraph rather than re-deriving the principle.
...
The derivation above relies entirely on $z_k = h\,x_k + v_k$ with $h$ constant (or a matrix $\mathbf{H}$ in the vector case). That is what lets us stay Gaussian through the update step and write a closed-form recursion for the mean and variance. The moment the observation function becomes genuinely non-linear, we lose that, and it is not really a fixable problem at the Kalman filter level --- there is no principled way to plug a non-linear $h(x)$ into the update equations. People do it anyway by replacing $h(x)$ with some linear surrogate, but at that point we are not running the Kalman filter on a non-linear model, we are running it on a linearized model we picked ourselves, and the answer is only as good as that choice. Since in practice almost every model we care about is non-linear, we really do need a generalization, and in the next section we will discuss two of them: the Extended Kalman Filter (EKF) and the Unscented Kalman Filter (UKF).

None of this makes the Kalman filter useless. On the contrary, being exact on linear Gaussian models is exactly what we want for one important use later on. When we build the differentiable particle filter and run HMC on top of it in Section~\ref{sec:param-estimation}, we need a way to check that the HMC pipeline itself is correct before we trust it on anything harder. The 1-D linear Gaussian model is the obvious test: the Kalman filter gives us the exact marginal log-likelihood, so it gives us the exact posterior over whatever parameter we are estimating. If HMC on the same model with the same data recovers that posterior, we know the pipeline is sound; if it does not, the problem is in HMC, not in the filter. We will come back to this cross-check in Section~\ref{sec:linear-gaussian-benchmark} when we benchmark the differentiable pipeline.

\paragraph{Demonstration on a 1D linear Gaussian.}
To make the ``Kalman is exact for linear Gaussian'' claim concrete, we run the filter on the exact 1D model that will reappear in the HMC validation in \S\ref{sec:linear-gaussian-benchmark}. The dynamics are $x_k = 0.95\,x_{k-1} + w_k$ with $w_k \sim \mathcal{N}(0, 0.25)$ ($B = 0.5$), observation $z_k = x_k + v_k$ with $v_k \sim \mathcal{N}(0, 0.09)$ ($D = 0.3$), prior $\mathcal{N}(0, \Sigma_0)$ with $\Sigma_0$ solved from the discrete Lyapunov equation following \S\ref{par:init-conditions}, seed $42$, $T = 100$ time steps, and 64-bit precision. The Kalman gain converges to its steady-state value $K \approx 0.777$ within the first five time steps, confirming the Riccati recursion has reached its fixed point. Figure~\ref{fig:kalman-1d-linear} shows the resulting trajectory.

We report two metrics throughout. The \emph{root-mean-square error} (RMSE) of the filter's posterior mean against the true state trajectory,
\begin{equation}
...
\end{split}
\end{equation}
by weak nonlinearality, I meant when the second derivative term can be ignored. We would expect such linear approximation to fail when the following two terms are comparable.
\begin{equation}
\begin{split}
...
We define the Range bearing model and 1-d Stochastic volatility model here. This is the first time they appear and they will also be used in later sections.
...
\end{equation}
with persistence parameter $\alpha = 0.91$ and volatility-of-volatility $\sigma = 1.0$. Since $|\alpha| < 1$, the process is stationary, and we initialize $x_0 \sim \mathcal{N}\!\left(0,\, \sigma^2/(1-\alpha^2)\right)$ from its stationary distribution, following the same principle discussed for the 1-D Kalman filter in Section~\ref{sec:kalman-algorithm}. The observation model is
\begin{equation}
    y_t = \beta\,\exp\!\left(\tfrac{x_t}{2}\right) v_t, \qquad v_t \sim \mathcal{N}(0, 1),
\end{equation}
with scale parameter $\beta = 0.5$. Note that $p(y_t \mid x_t) = \mathcal{N}(0,\, \beta^2 e^{x_t})$, so the observation noise is multiplicative and the likelihood is non-Gaussian in $x_t$. It is straightforward to run EKF and UKF on the range bearing model. Stochastic volatility is harder because the multiplicative noise means the observation is not a function of $x_t$ alone in the additive sense EKF/UKF assume. To make EKF/UKF applicable, we apply a log-squared transform $z_t = \log y_t^2$ to the observation, which yields
\begin{equation}
...
\paragraph{Computational footprint.} All six runs execute on CPU; the EKF/UKF machinery has no meaningful GPU workload at $d_x \le 2$ and $T = 100$. The numbers below are measured on the same host as the Kalman demo in \S\ref{sec:kalman-algorithm} and should be read as a reference point for the particle-filter wall times in Tables~\ref{tab:edh-rb}--\ref{tab:edh-sv-log}, not as an optimisation target.
...
Sequential use is where importance sampling becomes a filter. Suppose at time $k-1$ we hold a particle approximation $\{x_{k-1}^{(i)}, w_{k-1}^{(i)}\}$ of $p(x_{k-1} \mid z_{1:k-1})$. Our \textit{only} goal at time $k$ is the posterior $p(x_k \mid z_{1:k})$, whose mean $m_k$ and covariance $P_k$ are the quantities of interest. The intermediate predicted distribution $p(x_k \mid z_{1:k-1})$ and its moments $m_k^-$, $P_k^-$ are not ends in themselves—they are merely one route to the posterior. This gives us great freedom: we may draw new particles from \textit{any} proposal
\begin{equation}
...
We run every variant on all three headline models at $N = 200$ particles, $T = 100$ time steps, and $n_\lambda = 29$ flow steps with the geometric ratio $q = 1.2$ unless noted.
...
\centering
\footnotesize
\caption{Stochastic volatility, \emph{log-squared-transformed} model ($z = \log y^2$; $T=100$, $N=200$). The linear transform restores competitiveness to EKF/UKF and the flow family; kernel is slower but still tracks. LEDH-invertible narrowly beats the bootstrap PF on RMSE here, at $6\times$ the wall-time.}
...
The SV takeaway: on the raw model, every filter that relies on a local linearisation of $h$ (EKF, UKF, EDH, LEDH, kernel) fails at the same ${\sim}2.0$ RMSE, and only the bootstrap PF --- which evaluates the likelihood directly without touching $h$'s Jacobian --- tracks the latent volatility. The log-squared transform flattens the ranking: RMSE compresses into a ${\sim}1.14$--$1.16$ band and the wall-time differences become the discriminator. EKF and UKF are again the cheap-and-good option; the flow filters are not wrong on this model, just unnecessary.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{pf_edh_ledh_sv_log.png}
    \caption{Log-squared stochastic volatility: bootstrap PF (blue), EDH flow (green), and LEDH flow (orange) compared against the true log-volatility (black) with back-projected observations as red crosses. After the log-squared transform, the three filters compress into a narrow band and the differences between them become invisible at this scale; the three flow variants all land in the same narrow RMSE band as the LEDH-invertible entry in Table~\ref{tab:edh-sv-log}.}
...
Before the SV2D model reappears in \S\ref{sec:sv2d} as an HMC gradient-failure case, I record the filter-level result first. The three figures below compare bootstrap PF, EDH flow, LEDH flow, and the invertible counterparts of the flow variants on an SV2D trajectory with $N = 200$ particles and $T = 100$ time steps. A clean filter result on this model does not imply HMC will succeed on it --- that is the point of \S\ref{sec:sv2d} --- but a broken filter result would imply HMC cannot possibly succeed.
...
The Jacobian correction is harmless on range-bearing and should help on high-dimensional targets, but on acoustic tracking it is strictly worse than uncorrected LEDH in my runs. I defer the root-cause investigation to future work --- the most likely culprits are float32 precision on the per-particle Cholesky and the log-determinant clipping I use for stability.
...
Before we get into details, one framing up-front: OT resampling's primary value in this report is \emph{differentiability}, not lower filtering RMSE. Table~\ref{tab:soft-vs-ot-sv} in \S\ref{par:soft-vs-ot} will show OT paying a $20$--$180\times$ compute tax on stochastic volatility without beating stop-gradient systematic or soft resampling on RMSE. The motivation for paying that tax only lands in \S\ref{sec:param-estimation}, where the gradient surface determines whether HMC can recover the parameter posterior.

Previously, we have discussed the sample impoverishment problem introduced by low quality resampling. Therefore, we need to think about how to improve it. Before and after resampling, we are preserving the distribution. In \cite{reich2013nonparametric}, the author proposed to use optimal transport to do the resampling. An easy way to see this is that, before the resampling, we have particle location and weights at $\{\omega_i, x_i\}$, and after the resampling, we have particle location and weights at $\{\tilde\omega_i, \tilde x_i\}$. If we think $x_i$ are basis vectors and $\omega_i$ are the coefficients, we are dealing with a linear transformation problem. The transformation constructs an equal-weight approximation that reduces resampling variance and better preserves the structure of the distribution. 

To put it sharply, we want to make sure $\{\omega_i, x_i\}$ and $\{\tilde\omega_i, \tilde x_i\}$ represent the same distribution with $\tilde\omega_i \equiv \frac1N$, where $N$ is the total number of particles.There are two problems on resampling that we need to address separately:
...
\paragraph{Convention used in the algorithm below.} With the marginals as above, the optimal coupling $\mathbf{P}^*$ has row sums $\mathbf{w}$ (source) and column sums $\tfrac{1}{N}\mathbf{1}_N$ (target). The resampled particles, following Reich~\cite{reich2013nonparametric}, are $\hat{\mathbf{x}}^{(i)} = (1/b_i)\sum_j P^*_{ji}\,\mathbf{x}^{(j)} = N\sum_j P^*_{ji}\,\mathbf{x}^{(j)}$, so each new particle is a convex combination of the old ones with column-wise weights of $\mathbf{P}^*$. In the implementation we build a transport matrix $T$ such that $\hat{\mathbf{x}}^{(i)} = \sum_j T_{ij}\mathbf{x}^{(j)}$, which means $T_{ij} = N\,P^*_{ji}$: the code's $T$ is $N$ times the \emph{transpose} of the coupling $\mathbf{P}^*$. The `normalise columns / apply source weights' block in the OT resampling algorithm below is the step that converts $\mathbf{P}^*$ into this $T$. We flag the convention explicitly because row-vs-column and source-vs-target are easy to swap silently when translating between the Reich formula and the Sinkhorn output.
...
\STATE \textbf{Single-step extrapolation} (approximate autodiff window; \emph{not} the implicit function theorem route --- see \S\ref{sec:sinkhorn-backward}):
...
\centering
\footnotesize
\caption{Resampling comparison on stochastic volatility (1D, $T=500$, $N=200$). Headline: OT adds roughly $20$--$180\times$ wall-time vs.\ systematic / soft, with no RMSE improvement. Soft resampling ties stop-gradient on RMSE at the same cost. Systematic is the cheapest and has the best RMSE of this group; OT's value is gradient quality under HMC (\S\ref{sec:param-estimation}), not filtering accuracy.}
...
\REQUIRE Upstream gradient $\partial L / \partial \mathbf{T}$ from autodiff
...
I ran $42$ implicit-backward gradient checks. Each case differentiates $\log p(z_{1:T}\mid\v\theta)$ with respect to one scalar parameter. The autodiff gradient is the derivative produced by automatic differentiation through the whole differentiable filter pipeline, with Sinkhorn resampling using the implicit backward pass. The finite-difference reference uses central differences with base step $10^{-4}$ and five radii; the finite-difference median is the median of those five slopes. The relative error is
\begin{equation}
    \frac{\left|d_{\mathrm{autodiff}}-d_{\mathrm{FD}}\right|}
...
\toprule
$T$ & $\alpha$ & FD slopes (5 radii) & FD median & autodiff & rel.\ err. \\
\midrule
...
The training pipeline is in place and runs end-to-end. The bugs I am chasing now are the map-quality and initialization issues flagged above. They do not break the training loop. They show up as a map that fits its training distribution but does not generalise in the way a resampler needs. The network can overfit. With the current collocation scheme and a single training cloud, the Monge--Amp\`ere residual can fall near zero without proving that the map transports unseen clouds correctly. I have not yet added the regularisation or validation split that would catch this. Fit quality is not operator quality yet.

\paragraph{Next step.}
The plan is to build a family of two-dimensional Gaussian clouds that look like what a filter would hand the resampler: different means, covariance scales and rotations, and weight profiles ranging from near-uniform to moderately skewed to strongly peaked. I will train on one subset and hold out the rest. The number to report is the Monge--Amp\`ere residual on the held-out Gaussians. That is the generalisation test I should have run before plugging the map into the filter. If training residuals are flat and held-out residuals stay large, the fix is architectural: stronger local-linearity regularisation, bandwidth annealing, wider context, or a denser cloud-sampling schedule.
...
\paragraph{Filtering accuracy is not gradient quality.} Section~\ref{sec:particle-methods-section} ranked filters by estimator accuracy: how close the filtered mean and covariance are to the true latent state, measured in RMSE and log-likelihood. This section ranks filters by \emph{gradient} quality: how well $\nabla_{\boldsymbol{\theta}} \log p_{\boldsymbol{\theta}}(z_{1:T})$ behaves under HMC. A filter can win the first ranking and lose the second, and vice versa --- a bootstrap filter with stop-gradient systematic resampling may track the state accurately yet hand HMC a discontinuous loss surface, while OT resampling often pays a $60$--$100\times$ compute tax that only shows up as useful gradient behaviour here. Keep the two criteria separate: §4 validates the filter as an estimator, §6 validates the filter as a differentiable subroutine in a parameter-inference pipeline.

\subsection{Markov Chain Monte Carlo}

\subsubsection{Metropolis-Hastings}

Our goal is to estimate the model parameters $\v \theta$ from observations $z_{1:T}$. We want either the MAP point or the full posterior $p(\v \theta \mid z_{1:T})$. The particle filter gives us the marginal likelihood $p(z_{1:T} \mid \v \theta)$, and combined with a prior $p(\v \theta)$, we have access to the log-posterior up to a constant. The concrete parameter-prior choices used in these runs are described in \S\ref{sec:param-prior-selection}.
\begin{equation}
...
When the pipeline is fully differentiable, we can do much better. We emphasise upfront: HMC is a method for sampling from the \emph{target it is given} --- the Metropolis correction compensates for numerical integrator error under that target, it does not cleanse a biased or noisy particle-filter marginal likelihood and turn it into an exact posterior. Whatever bias is present in the log-likelihood surface from, e.g., stop-gradient systematic resampling or the single-step Sinkhorn-backward approximation survives into HMC's stationary distribution. With that caveat, Hamiltonian Monte Carlo (HMC) \cite{Duane1987,Neal-2012-MCMC-Hamiltonian-Dynamics-ARXIV-1206-1901v1} treats parameter estimation as a physical dynamics problem. We introduce an auxiliary momentum variable $\v p$ of the same dimension as $\v \theta$ and define the Hamiltonian (total energy):
\begin{equation}
...
I ran MAP on four models as the pipeline smoke test: 1D linear Gaussian (where both BPF+OT and LEDH+OT were exercised so that I could verify the differentiable filter does not introduce a filter-family bias), range-bearing with LEDH+OT (two parameters, $\sigma_{\text{range}}$ and $\sigma_{\text{bearing}}$, estimated jointly), 1D stochastic volatility with LEDH+OT (estimating the persistence $\alpha$), and 2D stochastic volatility with LEDH+OT (estimating $\sigma_2$). Every run uses a single cosine warm-down learning-rate schedule shared across benchmarks; Figure~\ref{fig:map_learning_rate} confirms the schedules overlap exactly on the four runs, so the convergence differences that follow are not an artifact of tuning.
...
\begin{figure}[H]
    ...
    \caption{MAP convergence on 2D stochastic volatility with LEDH+OT, estimating $\sigma_2$ with true value $1.0$. The MAP optimizer converges on the filter-level objective, which is the minimum bar for the SV2D pipeline. As the HMC diagnostics in \S\ref{sec:sv2d} document, a clean MAP run on SV2D is \emph{necessary but not sufficient} for HMC to work --- the gradient surface is smooth enough on average for stochastic-gradient MAP to navigate, but has per-leapfrog-step discontinuities that derail fixed-seed HMC.}
    \label{fig:map_sv2d}
\end{figure}

\paragraph{What these four figures are for.}
These are the minimum bar every downstream benchmark must clear before HMC is run. On LG and range-bearing the MAP result is also a correctness check: the filter converges, the gradient path is not broken, and the true parameters are recovered. On SV1D the result is similar but noisier, as expected for a model where the observation does not directly reveal the latent. On SV2D the MAP convergence is the \emph{floor} of what the pipeline can do --- the HMC-specific failure mode described in \S\ref{sec:sv2d} is a strictly harder problem than MAP, and the contrast between the clean MAP trajectory here and the gradient-spike behaviour in \S\ref{sec:sv2d} is exactly the ``MAP tolerance does not imply HMC tolerance'' point made at the start of this subsection.

\subsection{Differentiable Resampling and the Gradient Chain}

The resampling derivative chain has already been analysed in \S\ref{sec:ot-derivative-chain}. We now test whether the differentiable filter delivers gradients usable for parameter inference: the theoretical properties in \S\ref{sec:ot-derivative-chain} are the predictions, the benchmarks below are the test.

\subsection{Parameter Prior Selection}
\label{sec:param-prior-selection}

\paragraph{What the prior is doing here.}
There are two different priors in this section, and I keep them separate. The filter has an initial-state prior over $x_0$; that is the Lyapunov or physical prior described in \S\ref{par:init-conditions}. HMC has a parameter prior $p(\v\theta)$ over the unknown constants that I am estimating. This subsection is about the second object. The posterior target in the code is not just the particle-filter likelihood. It is
\begin{equation}
    \log p(\v\theta \mid z_{1:T}) =
    \log p(z_{1:T} \mid \v\theta) + \log p(\v\theta) + \mathrm{const}.
\end{equation}
...
We did not apply these updates to the runs in this report due to time constraints. The $\sigma_2$-only configuration has been partially mode-corrected but the sibling configurations and the guide table itself have not been synced. A future iteration should (i) switch linear-Gaussian observation-noise priors to $\mathrm{LogNormal}(0.25, 0.5)$, (ii) replace the SV2D $\mathrm{Uniform}(0,1)$ persistence prior with $\mathrm{Beta}(10, 2)$ via a scaled logistic transform, (iii) propagate the $\sigma_2$ mode-accurate loc $=0.25$ to all SV2D HMC configurations, and (iv) re-run the linear-Gaussian and SV2D benchmarks under the updated priors. The SV2D failure mode in \S\ref{sec:sv2d} may partially lift under the informative $\mathrm{Beta}(\alpha,\beta)$ prior, but we cannot claim so without rerunning.

\subsection{Benchmark: Linear Gaussian Model}
\label{sec:linear-gaussian-benchmark}

We use the linear Gaussian model as the primary validation target. The filters that HMC calls as subroutines use the Lyapunov-stationary initial prior $\mathcal{N}(\mathbf{0}, \boldsymbol{\Sigma}_0)$ with $\boldsymbol{\Sigma}_0$ solving $\boldsymbol{\Sigma}_0 = \mathbf{F}\boldsymbol{\Sigma}_0\mathbf{F}^\top + \mathbf{Q}$, following the cross-cutting principle in \S\ref{par:init-conditions}. The Kalman filter gives the exact posterior, so we have ground truth for both the filter estimates and the parameter posterior.
...
\paragraph{HMC trace.}
Figure~\ref{fig:hmc_lg_trace} overlays the post-burn-in chains from all five filters on a single axis. Kalman (the analytic exact filter), EKF, UKF, LEDH+OT, and BPF+OT all target the same parameter $\sigma_{\text{obs}} = 1.0$ with the same HMC step-size schedule and $L = 5$ leapfrog steps per iteration. The trace shows that every chain stays inside a neighbourhood of the true value; none of the five filters drift systematically away from truth or get stuck at a boundary.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{hmc_lg_trace.png}
    \caption{HMC post-burn-in traces for $\sigma_{\text{obs}}$ on 1D linear Gaussian, overlaying all five filters. Kalman (black) is the analytic reference; EKF (blue), UKF (green), LEDH+OT (orange), and BPF+OT (purple) are the differentiable-filter variants. Horizontal dashed line at the true value $\sigma_{\text{obs}} = 1.0$.}
    \label{fig:hmc_lg_trace}
\end{figure}

\paragraph{HMC posteriors.}
Figures~\ref{fig:hmc_lg_histogram_kalman}--\ref{fig:hmc_lg_histogram_bpf_ot} show the per-filter marginal posterior histograms. Each panel plots the filter's own samples (coloured bars) against the reference outline from the Kalman filter's own HMC chain and the true value (dashed vertical line). The question each panel answers is: does this filter's HMC posterior match the Kalman-filter HMC reference?

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{hmc_lg_histogram_kalman.png}
    \caption{1D LG HMC posterior: Kalman filter. This is the HMC reference other filters are compared against.}
    \label{fig:hmc_lg_histogram_kalman}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{hmc_lg_histogram_ekf.png}
    \caption{1D LG HMC posterior: EKF. Kalman outline in grey, true value as dashed vertical.}
    \label{fig:hmc_lg_histogram_ekf}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{hmc_lg_histogram_ukf.png}
    \caption{1D LG HMC posterior: UKF.}
    \label{fig:hmc_lg_histogram_ukf}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{hmc_lg_histogram_ledh_ot.png}
    \caption{1D LG HMC posterior: LEDH+OT. This is the flow filter with the full three-term derivative chain active.}
...
Table~\ref{tab:hmc_diagnostics} collects the diagnostics for the 1D LG runs (and the other three nonlinear runs below). Kalman, EKF, and UKF all hit acceptance $\ge 0.97$ with ESS saturating at the sample count $200$ --- the low apparent split-$\hat R$ of $0.83$ is a finite-sample artefact of running a single $N=200$ chain and is not a convergence problem. LEDH+OT achieves acceptance $0.91$ with ESS $33.7$ over $400$ samples and split-$\hat R = 0.999$, which is the cleanest "non-trivial" result: the flow filter with the full three-term derivative chain matches the Kalman posterior shape. BPF+OT lands at split-$\hat R = 1.15$, above the standard non-convergence threshold of $1.1$ --- its chain did not mix well enough at this sample count, which is visible in Figure~\ref{fig:hmc_lg_histogram_bpf_ot} as an asymmetric histogram relative to the Kalman outline. The BPF+OT result is therefore a known gap: the gradient chain is correct (§\ref{sec:sinkhorn-backward}), the MAP converges cleanly (§\ref{sec:map-smoke-test}), but the HMC chain needs either more samples, a longer burn-in, or step-size re-tuning before the posterior can be trusted. I record the run here rather than omit it --- the diagnostic is what tells us the chain is not ready.

\subsubsection{Multi-Dimensional Model, 1 Parameter}
\label{sec:multi-d-lg}
We increase the state dimension to $d \in \{3, 5\}$ while still estimating a single scalar parameter $\theta$. Filter initial conditions remain Lyapunov-stationary per \S\ref{par:init-conditions}, which generalises cleanly to the multi-dimensional case via the discrete Lyapunov equation. The purpose of this subsection is narrow: rule out state-dimension scaling as a confounder for the SV2D gradient failure in \S\ref{sec:sv2d}. If the linear pipeline still converges at $d = 5$ --- MAP hits the truth, HMC matches the Kalman analytic posterior, gradient norms stay bounded across leapfrog steps --- then whatever breaks on SV2D is a nonlinearity effect, not a dimension effect.

The true posterior over $\theta$ remains one-dimensional and analytically tractable via the Kalman filter at any state dimension, so the multi-D benchmark has the same ground-truth reference as \S\ref{sec:linear-gaussian-benchmark}. We report the same diagnostics table (acceptance rate, ESS, split-$\hat R$, step size, wall-time) as the 1D run, for $d = 3$ and $d = 5$. Two short paragraphs cover the scaling observations: (i)~how ESS and wall-time change with state dimension at fixed particle count, and (ii)~whether the adaptive step size stabilises at a comparable value or has to shrink with $d$. We do not repeat the MAP validation plots or the trace diagnostics here --- those are in \S\ref{sec:linear-gaussian-benchmark}, and their point on the multi-D runs is only that nothing qualitatively new appears.
...
The diagnostic readout is the red flag in this section. Acceptance rate $0.77$ is fine, but ESS is $\mathbf{4.0}$ over $500$ samples --- effectively no independent information is extracted from the chain, it is moving in a tight oscillation that happens to pass the acceptance test without exploring the posterior. Split-$\hat R = 0.66$ is below $1$ by enough to suggest the chain is trapped rather than mixing. The posterior in Figure~\ref{fig:hmc_sv1d_histogram} therefore represents a very tight local region, not a faithful marginal over $\alpha$. I include this run as an honest diagnostic: the filter is fine (\S\ref{sec:particle-methods-section} shows LEDH+OT tracks SV1D at $\mathrm{RMSE} \approx 1.16$) and the gradient path is validated (Table~\ref{tab:gradient_validation}), but the HMC chain did not mix at the sample budget and step-size tuning I used here. This run is a known gap.

\paragraph{Diagnostic summary table.}
Table~\ref{tab:hmc_diagnostics} collects the diagnostics for all eight HMC runs in this report --- the five 1D LG runs above, plus the two nonlinear runs just described, plus the SV2D run discussed in \S\ref{sec:sv2d}.

\begin{table}[H]
\centering
\footnotesize
\caption{HMC diagnostics for all eight runs. ``Acceptance'' is the fraction of leapfrog proposals accepted; ``ESS'' is effective sample size at the reported $N_{\text{samples}}$; ``split-$\hat R$'' is the single-chain split potential-scale-reduction factor (values near $1$ are clean, values $> 1.1$ flag non-convergence, very-low values can flag a trapped chain). ``Wall'' is total burn-in + sampling wall time; ``Time/step'' is seconds per HMC iteration ($L = 5$ leapfrog steps per iteration for every run). Memory is not reported --- the current HMC logging does not store peak memory.}
\label{tab:hmc_diagnostics}
\begin{tabular}{lcccccccc}
\toprule
Model / filter & Acceptance & ESS & split-$\hat R$ & Wall (s) & Time/step (s) & $N_{\text{samples}}$ & $N_{\text{burnin}}$ & $L$ \\
\midrule
LG Kalman    & $0.98$ & $200.0$ & $0.826$ & $2{,}077$  & $6.92$  & $200$ & $100$ & $5$ \\
LG EKF       & $0.98$ & $200.0$ & $0.826$ & $2{,}086$  & $6.95$  & $200$ & $100$ & $5$ \\
LG UKF       & $0.97$ & $200.0$ & $0.849$ & $5{,}337$  & $17.79$ & $200$ & $100$ & $5$ \\
LG LEDH+OT   & $0.91$ & $33.7$  & $0.999$ & $4{,}610$  & $7.68$  & $400$ & $200$ & $5$ \\
LG BPF+OT    & $0.87$ & $17.6$  & $1.154^{*}$ & $1{,}216$  & $2.03$  & $400$ & $200$ & $5$ \\
...
\end{tabular}
\\\vspace{2pt}
{\footnotesize $^{*}$Diagnostic red flag. BPF+OT on LG has split-$\hat R > 1.1$ (non-convergence threshold); SV1D LEDH+OT has ESS $= 4$ effectively zero independent samples and split-$\hat R \ll 1$ suggesting a trapped chain. Both are recorded honestly rather than omitted; neither represents a faithful posterior.}
\end{table}

Four of the eight runs are cleanly converged (all three LG Kalman-family runs plus LG LEDH+OT). RB LEDH+OT and SV2D LEDH+OT are converged on the standard threshold but sit at relatively low ESS given their sample budgets. LG BPF+OT and SV1D LEDH+OT are the two red flags, for different reasons: BPF+OT has a mixing problem on LG (split-$\hat R = 1.15$), and SV1D has a trapped chain (ESS $= 4$). These gaps are the reason the SV2D failure mode discussed in \S\ref{sec:sv2d} is flagged as an \emph{open} problem rather than a clean negative result.
...
Before discussing the symptom and hypotheses, the post-burn-in HMC chain is shown in Figures~\ref{fig:hmc_sv2d_trace} and~\ref{fig:hmc_sv2d_histogram} with its diagnostic entry in Table~\ref{tab:hmc_diagnostics}. Acceptance is $0.75$ and split-$\hat R$ is $1.02$ --- the aggregate diagnostics look benign. The failure mode described below is step-level, not chain-aggregate: the gradient norm spikes at specific leapfrog substeps while the overall acceptance rate stays above target, so an aggregate-only look at this run would miss it.
...
SV2D integration of the implicit Sinkhorn backward & Theoretical: §\ref{sec:sinkhorn-backward} shows the extrapolation is known to be biased for Term 1, and SV2D activates that term strongly. & The implicit backward passes the filter-level gradient checks in §\ref{sec:ot-derivative-chain}; what remains untested is the full SV2D HMC inner loop. & Open \\
...
\emph{The linear Gaussian benchmark in \S\ref{sec:linear-gaussian-benchmark} validates the HMC machinery: MAP converges, gradients match finite differences, HMC posteriors match the Kalman reference. On SV2D, gradient spikes during leapfrog integration cause acceptance-rate collapse. I have localised the failure to the nonlinear filter gradient path but have not isolated the root cause. The implicit Sinkhorn backward is derived and validated at the filter-gradient level in \S\ref{sec:ot-derivative-chain}; what I have not done is run the SV2D HMC inner loop with that backward and a branch-free conditional-resampling path. This limits the nonlinear DPF-HMC claims but does not affect the filtering results in \S\ref{sec:particle-methods-section} or the validation of the linear pipeline.}

\subsection{Limitations}

To collect the caveats that have been raised section-by-section: (i)~the differentiable-filter HMC pipeline is validated on linear Gaussian targets only; on SV2D the pipeline hits an unresolved gradient-spike failure documented in \S\ref{sec:sv2d}, so we make no parameter-inference claims on the nonlinear benchmark; (ii)~$\hat{R}$ in this report is a single-chain split $\hat R$, not a true multi-chain diagnostic; (iii)~the implicit Sinkhorn backward is validated as a filter-gradient check, but the SV2D HMC runs reported here were not rerun with that backward inside the full leapfrog loop.
% 
% % \newpage
% 
% 
