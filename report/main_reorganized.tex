...
To collect the caveats that have been raised section-by-section: (i)~the differentiable-filter HMC pipeline is validated cleanly on linear Gaussian targets and demonstrated on range-bearing, but it is not yet reliable on SV1D or SV2D --- the SV1D chain is trapped (ESS $=4.0$) and the SV2D chain has the step-level gradient spikes described in \S\ref{sec:sv2d}; (ii)~$\hat{R}$ in this report is a single-chain split $\hat R$, not a true multi-chain diagnostic.
% 
% % \newpage
% 
% 
